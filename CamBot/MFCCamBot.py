...
// @version      1.0
// @description  AI cam assistant for MyFreeCams with Discord control and tone-aware suggestions
...
(function () {
    ...
    const discordWebhook = "https://discord.com/api/webhooks/1379934755314864278/q-0PnYE18smCoX7n3V7WjcIgQ_46bVLSllI-f9Nx-RwKk60J3Q5g7FHIzdxphgGDYTjg";
    ...
    function postToDiscord(text) {
        GM_xmlhttpRequest({
            ...
            data: JSON.stringify({
                ...
                content: text ? `💬 **[MFC] Ava Suggestion**\n${text}` : "⚠️ Ava responded but the suggestion was empty or invalid."
            })
        });
    }

    function observeChat() {
        const iframe = document.querySelector('iframe[src*="chat"]');
        if (!iframe) {
            console.log("❌ MFC chat iframe not found");
            return;
        }

        iframe.addEventListener("load", () => {
            const chatDoc = iframe.contentDocument || iframe.contentWindow.document;
            const chatBox = chatDoc.querySelector('body');

            if (!chatBox) {
                console.log("❌ MFC chat body not found");
                return;
            }

            console.log("✅ Ava is now watching chat on MFC...");
